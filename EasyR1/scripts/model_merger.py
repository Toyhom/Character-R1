...
import shutil  # 导入 shutil 以便复制文件
...
if __name__ == "__main__":
    ...
    parser.add_argument("--hf_source_dir", default=False, type=str, help="The path of the original huggingface model")
    ...
    src_tokenizer_config = os.path.join(args.hf_source_dir, "tokenizer_config.json")
    if os.path.exists(src_tokenizer_config):
        shutil.copy2(src_tokenizer_config, hf_path)
        print(f"已成功覆盖 tokenizer_config.json。")
    else:
        print(f"警告: 在 {args.hf_source_dir} 中未找到 tokenizer_config.json。")
