...
class RewardScaler:
    """
    通过维持运行中的均值和标准差来归一化奖励。
    """
    def __init__(self, shape, gamma=0.99, epsilon=1e-8):
        self.shape = shape  # shape 应该是 (num_rewards,)
        self.gamma = gamma
        self.epsilon = epsilon
        # 使用 torch.Tensor 并将其保留在 CPU 上
    ...
    def update(self, rewards: torch.Tensor):
        """
        用新一批的奖励来更新运行均值和方差。
        rewards: 一个形状为 (batch_size, num_rewards) 的 Tensor。
        """
        # 确保计算在 CPU 和 float64 上进行以保证精度
        ...
        # 使用指数移动平均（Exponential Moving Average）来更新
...
class CharacterGroupedRewardScaler:
    """
    一个管理器，为每个角色名称（字符串）动态创建并维护一个独立的 RewardScaler。
    原生支持将角色名称作为 np.ndarray(dtype=object) 传入。
    """
    ...
    def update_and_scale(self, rewards: torch.Tensor, role_names: np.ndarray) -> torch.Tensor:
        """
        一个核心方法，根据角色名称对奖励进行分组、更新和归一化。
        
        Args:
            rewards (torch.Tensor): 形状为 (batch_size, num_rewards) 的原始奖励张量。
            role_names (np.ndarray): 形状为 (batch_size,) 的NumPy数组，dtype=object，
                                     其中每个元素是角色的字符串名称。

        Returns:
            torch.Tensor: 形状为 (batch_size, num_rewards) 的、经过条件归一化的奖励张量。
        """
        # 使用 .shape[0] 更符合NumPy的习惯
        if role_names.shape[0] != rewards.shape[0]:
            raise ValueError(f"rewards 的批次大小 ({rewards.shape[0]}) 与 role_names 的长度 ({role_names.shape[0]}) 不匹配。")

        scaled_rewards = torch.empty_like(rewards)
        
        # set() 可以直接作用于 NumPy 数组，高效地获取唯一值
        unique_roles_in_batch = set(role_names)

        for role_name in unique_roles_in_batch:
            if role_name not in self.scalers:
                print(f"首次遇到角色: '{role_name}'，为其创建新的 RewardScaler。")
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        # [新增] 初始化奖励归一化器和权重
        # 这个列表需要和你 reward_fn 返回的奖励名称完全一致
        self.reward_names = [
        ...
        self.grouped_scaler = CharacterGroupedRewardScaler(shape=(len(self.reward_names),), gamma=0.99, reward_names=self.reward_names)


        self.reward_scaler = RewardScaler(shape=(len(self.reward_names),))
    ...
    def _load_checkpoint(self) -> None:
        ...
        # 检查原始路径是否是一个目录，并且本身不是一个checkpoint文件夹
        # 这样，如果用户直接指定了 '.../global_step_10000'，这段逻辑会跳过，尊重用户的精确指定
        if os.path.isdir(base_path) and 'global_step_' not in os.path.basename(base_path):
            print(f"'{base_path}' 是一个目录，将自动搜索最新的 checkpoint...")
            
            # 1. 找出所有 'global_step_*' 格式的子文件夹
            try:
                subfolders = [
                ...
                raise FileNotFoundError(f"指定的 checkpoint 目录不存在: {base_path}")
            if not subfolders:
                raise FileNotFoundError(f"在目录 '{base_path}' 中没有找到任何 'global_step_*' 格式的 checkpoint 文件夹。")
            # 2. 按 step 数值排序（从大到小）
            # 使用 lambda 函数提取 'global_step_' 后面的数字并转为整数，以确保正确排序
            # 例如，它能正确处理 global_step_10000 > global_step_2000
            subfolders.sort(key=lambda x: int(x.split('_')[-1]), reverse=True)
            
            # 3. 获取最新的文件夹，并更新配置中的路径
            ...
            print(f"已自动选择最新的 checkpoint: {self.config.trainer.load_checkpoint_path}")
            ...
            for batch_dict in tqdm(self.train_dataloader, desc="Running step", position=1):
                ...
                with timer("step", timing_raw):
                    ...
                    with timer("adv", timing_raw):
                        ...
                        if self.config.algorithm.reward_scaler:

                            reward_tensors_dict, reward_metrics = ray.get(reward_ref)

                            del reward_tensors_dict["response_length"]
                            del reward_tensors_dict["overall"]
                            
                            # 2. 将奖励字典转换为一个 (batch_size, num_rewards) 的张量
                            #    注意：这里的顺序必须和 self.reward_names 中定义的一致！
                            device = list(reward_tensors_dict.values())[0].device
                            
                            # 假设每个奖励张量形状为 (batch_size, seq_len)
                            # 我们需要先对 seq_len 维度求和，得到每个样本的总分
                            # 如果你的奖励函数已经是 per-sequence 的，则不需要 .sum(dim=-1)
                            rewards_per_sequence = torch.stack(
                                [reward_tensors_dict[name].sum(dim=-1) for name in self.reward_names],
                                dim=1
                            ) # shape: (batch_size, num_rewards)

                            # 3. 更新 scaler 并获取归一化后的奖励

                            if self.config.algorithm.reward_scaler_method == "role":
                                scaled_rewards = self.grouped_scaler.update_and_scale(rewards_per_sequence, batch.non_tensor_batch["role_name"])
                                all_stats = self.grouped_scaler.get_all_stats()
                                metrics.update(all_stats)
                            elif self.config.algorithm.reward_scaler_method == "group":
                                scaled_rewards = self.grouped_scaler.update_and_scale(rewards_per_sequence, batch.non_tensor_batch["role_group"])
                                all_stats = self.grouped_scaler.get_all_stats()
                                metrics.update(all_stats)                                
                            else:
                                # record_scaled_rewards = self.grouped_scaler.update_and_scale(rewards_per_sequence, batch.non_tensor_batch["role_name"])
                                # all_stats = self.grouped_scaler.get_all_stats()
                                # metrics.update(all_stats)

                                self.reward_scaler.update(rewards_per_sequence)
                                scaled_rewards = self.reward_scaler.scale(rewards_per_sequence) # shape: (batch_size, num_rewards)
                                # 记录归一化后奖励的统计信息
                                metrics["reward/scaled_mean"] = scaled_rewards.mean().item()
                                metrics["reward/scaled_std"] = scaled_rewards.std().item()

                        
                            # 归一化之后的奖励
                            scaled_reward_metrics = {f"reward/scaled_{k}": v for k,v in zip(self.reward_names, scaled_rewards.mean(dim=0).tolist())}
                            metrics.update(scaled_reward_metrics)

                            # 4. 使用权重将归一化奖励合并为最终的 per-sequence 奖励
                            self.reward_weights = self.reward_weights.to(scaled_rewards.device)
                            final_sequence_reward = (scaled_rewards * self.reward_weights).sum(dim=-1) # shape: (batch_size,)

                            # 5. 将 per-sequence 奖励广播回 token 级别
                            response_mask = batch.batch["response_mask"].to(device)
                            
                            # 创建一个全零的 token 级奖励张量
                            token_level_scores = torch.zeros_like(response_mask, dtype=torch.float32)
                            
                            # 找到每个序列的最后一个有效 token 的索引
                            last_token_indices = response_mask.sum(dim=1).long() - 1
                            last_token_indices = last_token_indices.clamp(min=0) # 防止空序列导致索引为-1
                            
                            # 将最终的 sequence reward 赋值给最后一个 token
                            batch_indices = torch.arange(token_level_scores.size(0), device=device)
                            token_level_scores[batch_indices, last_token_indices] = final_sequence_reward.to(torch.float32)
